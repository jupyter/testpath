...
class MockCommand(object):
    ...
    def fixed_output(cls, name, stdout="", stderr="", exit_status=0):
        ...
        """
        return cls(name, python=_output_template.format(stdout, stderr, exit_status))
    ...
    def __enter__(self):
        if os.path.isfile(self._cmd_path):
            raise EnvironmentError(
                "Command %r already exists at %s" % (self.name, self._cmd_path)
            )
    ...
    def assert_called(self, args=None):
        ...
        if args is not None:
            if not any(args == c["argv"][1:] for c in calls):
                msg = [
                    "Command %r was not called with specified args (%r)"
                    % (self.name, args),
